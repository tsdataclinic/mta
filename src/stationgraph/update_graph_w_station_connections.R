...
option_list = list(
    ...
    make_option(c("--stopids"), type="character", default=NULL, 
...
p_gtfs <- read.csv(opt$stopids,stringsAsFactors = F)

station_connections <- merge(ss[,c('from','to','line')],p_gtfs[,c('platform_id','stop_id')],by.x = 'from',by.y='stop_id')
colnames(station_connections)[4] <- 'from_pid'
station_connections <- merge(station_connections,p_gtfs[,c('platform_id','stop_id')],by.x = 'to',by.y='stop_id')
