...
def collate(updates, include_alerts=False):
    ...
    while at_least_one_match_found:
        ...
        for uid in interim:
            ...
            for candidate_uid in possible_matches:
                ...
                if len(current_first_remaining_stop) == 0:
                    continue
                current_first_remaining_stop = current_first_remaining_stop[0]['stop_id']

                candidate_initial_stop = interim[candidate_uid][0]['trip_update']\
                    ['trip_update']['stop_time_update']
                if len(candidate_initial_stop) == 0:
                    continue
                candidate_initial_stop = candidate_initial_stop[0]['stop_id']
    ...
    # The ID generated by collate thus far is a uuid value which is time and machine dependent,
    # but is convenient as a stopgap value. Our next step is to switch to using a uniqified
    # version of the true trip_id.
    for key in list(out.keys()):
        trip_id = out[key][0]['trip_update']['trip_update']['trip']['trip_id']
        timestamp = out[key][0]['timestamp']
        uid = trip_id + '_' + str(timestamp)
        out[uid] = out.pop(key)
...
def logify(updates):
    ...
    # collate generates `unique_trip_id` values for each trip and keys them to message
    # collections.
    ...
    for unique_trip_id in message_collections:
        ...
        #print(f"message_collection {message_collection}")
        action_logs = _parse_message_list_into_action_logs(
            message_collection, message_timestamps
        )
        #print(f"action_logs {action_logs}")

        if all(df.empty for df in action_logs):
            continue
    ...
    return logbook, timestamps, parse_errors, action_logs
